...
export class JobScheduler {
     ...
     */
    scheduleWeightedFairShare(jobs) {
        // Group jobs by agent
        const agentJobs = new Map();
        jobs.forEach((job) => {
            if (!agentJobs.has(job.agentId)) {
     ...
     */
    calculateEffectivePriority(job, ageMultiplier) {
     ...
     */
    decrementAgentJobCount(agentId) {
        const currentCount = this.agentJobCounts.get(agentId) || 0;
