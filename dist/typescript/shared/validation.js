...
import { BasicPromptInjectionDetector, } from '../../../../../riatzukiza/promethean/packages/security/dist/testing/prompt-injection.js';
/**
...
export class InputValidator {
     ...
     */
    async validateJobSubmission(jobData) {
        const errors = [];
        const warnings = [];
        ...
        if (jobData.jobType && !validJobTypes.includes(jobData.jobType)) {
            errors.push(new ValidationError(`Invalid job type: ${jobData.jobType}. Must be one of: ${validJobTypes.join(', ')}`, 'jobType', 'INVALID_VALUE'));
        }
        // Validate priority
        const validPriorities = ['low', 'medium', 'high', 'urgent'];
        if (jobData.priority && !validPriorities.includes(jobData.priority)) {
            errors.push(new ValidationError(`Invalid priority: ${jobData.priority}. Must be one of: ${validPriorities.join(', ')}`, 'priority', 'INVALID_VALUE'));
        }
        ...
        if (jobData.modelName && this.config.allowedModelNames.length > 0) {
            if (!this.config.allowedModelNames.includes(jobData.modelName)) {
                errors.push(new ValidationError(`Model ${jobData.modelName} is not allowed`, 'modelName', 'FORBIDDEN_MODEL'));
            }
        }
        // Validate prompt content
        let sanitizedPrompt = jobData.prompt;
        let promptInjectionResult;
        if (jobData.prompt) {
    ...
    async validatePrompt(prompt) {
        ...
        if (prompt.length > this.config.maxPromptLength) {
            errors.push(new ValidationError(`Prompt too long: ${prompt.length} characters (max: ${this.config.maxPromptLength})`, 'prompt', 'TOO_LONG'));
            securityScore -= 0.3;
        }
        ...
        if (this.config.enablePromptInjectionDetection) {
            ...
            if (promptInjectionResult.detected) {
                ...
                if (promptInjectionResult.blocked) {
                    errors.push(new ValidationError('Prompt blocked due to security concerns', 'prompt', 'SECURITY_BLOCK'));
                    securityScore -= 0.6;
                }
            }
        }
    ...
    async validateMessages(messages) {
        ...
        if (messages.length > this.config.maxMessageCount) {
            errors.push(new ValidationError(`Too many messages: ${messages.length} (max: ${this.config.maxMessageCount})`, 'messages', 'TOO_MANY'));
            securityScore -= 0.3;
        }
        for (let i = 0; i < messages.length; i++) {
            const message = messages[i];
            if (!message.role || !message.content) {
                errors.push(new ValidationError(`Message ${i} missing required role or content`, `messages[${i}]`, 'MISSING_FIELDS'));
                continue;
            }
            const validRoles = ['user', 'assistant', 'system'];
            if (!validRoles.includes(message.role)) {
                errors.push(new ValidationError(`Invalid role in message ${i}: ${message.role}`, `messages[${i}].role`, 'INVALID_VALUE'));
            }
            // Validate message content
            const contentValidation = await this.validatePrompt(message.content);
            if (!contentValidation.isValid) {
                errors.push(...contentValidation.errors.map((err) => new ValidationError(err.message, `messages[${i}].content`, err.code)));
            }
    ...
    async validateInput(input) {
        ...
        if (typeof input === 'string') {
            if (input.length > this.config.maxPromptLength) {
                errors.push(new ValidationError(`Input too long: ${input.length} characters (max: ${this.config.maxPromptLength})`, 'input', 'TOO_LONG'));
                securityScore -= 0.3;
            }
            if (this.config.enableInputSanitization) {
            ...
            if (input.length > this.config.maxMessageCount) {
                errors.push(new ValidationError(`Too many input items: ${input.length} (max: ${this.config.maxMessageCount})`, 'input', 'TOO_MANY'));
                securityScore -= 0.3;
            }
            for (let i = 0; i < input.length; i++) {
                if (typeof input[i] !== 'string') {
                    errors.push(new ValidationError(`Input item ${i} must be a string`, `input[${i}]`, 'INVALID_TYPE'));
                    continue;
                }
                const item = input[i];
                if (item && item.length > this.config.maxPromptLength) {
                    errors.push(new ValidationError(`Input item ${i} too long: ${item.length} characters (max: ${this.config.maxPromptLength})`, `input[${i}]`, 'TOO_LONG'));
                    securityScore -= 0.2;
                }
            }
        }
    ...
    validateOptions(options) {
        ...
        if (options.temperature !== undefined) {
            if (typeof options.temperature !== 'number' ||
                ...
                errors.push(new ValidationError('Temperature must be a number between 0 and 2', 'options.temperature', 'INVALID_RANGE'));
            }
        }
        // Validate top_p
        if (options.top_p !== undefined) {
            if (typeof options.top_p !== 'number' || options.top_p < 0 || options.top_p > 1) {
                errors.push(new ValidationError('Top_p must be a number between 0 and 1', 'options.top_p', 'INVALID_RANGE'));
            }
        }
        // Validate num_ctx
        if (options.num_ctx !== undefined) {
            if (typeof options.num_ctx !== 'number' || options.num_ctx < 1 || options.num_ctx > 32768) {
                errors.push(new ValidationError('Num_ctx must be a number between 1 and 32768', 'options.num_ctx', 'INVALID_RANGE'));
            }
        }
        ...
        if (options.num_predict !== undefined) {
            if (typeof options.num_predict !== 'number' ||
                ...
                errors.push(new ValidationError('Num_predict must be a number between 1 and 32768', 'options.num_predict', 'INVALID_RANGE'));
            }
        }
        // Validate stop array
    ...
    validateAgentId(agentId) {
        ...
        if (!agentId || typeof agentId !== 'string') {
            errors.push(new ValidationError('Agent ID is required and must be a string', 'agentId', 'REQUIRED_FIELD'));
            return { isValid: false, warnings, errors, securityScore: 0 };
        }
        // Check for valid UUID format or similar identifier
        const validIdPattern = /^[a-zA-Z0-9_-]{8,64}$/;
        if (!validIdPattern.test(agentId)) {
            errors.push(new ValidationError('Agent ID must be 8-64 characters of letters, numbers, underscores, or hyphens', 'agentId', 'INVALID_FORMAT'));
        }
        return {
            isValid: errors.length === 0,
            warnings,
    ...
    validateSessionId(sessionId) {
        ...
        if (!sessionId || typeof sessionId !== 'string') {
            errors.push(new ValidationError('Session ID is required and must be a string', 'sessionId', 'REQUIRED_FIELD'));
            return { isValid: false, warnings, errors, securityScore: 0 };
        }
        // Check for valid UUID format or similar identifier
        const validIdPattern = /^[a-zA-Z0-9_-]{8,64}$/;
        if (!validIdPattern.test(sessionId)) {
            errors.push(new ValidationError('Session ID must be 8-64 characters of letters, numbers, underscores, or hyphens', 'sessionId', 'INVALID_FORMAT'));
        }
        return {
            isValid: errors.length === 0,
            warnings,
 ...
 */
export async function validateJobSubmission(jobData, config) {
 ...
 */
export async function validatePrompt(prompt, config) {
    const validator = new InputValidator(config);
